...
public class PenjualanController {
    ...
    public int tambahPenjualan(Penjualan p) {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            ps.setString(4, p.getStatusHutang());
            ps.setString(5, p.getNamaPelanggan());
            ps.executeUpdate();

            try (ResultSet rs = ps.getGeneratedKeys()) {
                if (rs.next()) {
                    return rs.getInt(1); // Kembalikan ID penjualan yang baru
                }
            }
    ...
    // Mengambil semua data penjualan (opsional)
    ...
    // Fungsi untuk mendapatkan penjualan berdasarkan status hutang
    ...
    //Update status penjualan jika berhutang
